...
EOF
  ...
  sudo VAULT_TOKEN=${VAULT_TOKEN} VAULT_ADDR="http://${VAULT_IP}:8200" vault policy write provisioner provisioner_policy.hcl

  # create a provisioner token
  PROVISIONER_TOKEN=`sudo VAULT_TOKEN=${VAULT_TOKEN} VAULT_ADDR="http://${VAULT_IP}:8200" vault token create -policy=provisioner -field=token`
  ...
  sudo VAULT_TOKEN=${VAULT_TOKEN} VAULT_ADDR="http://${VAULT_IP}:8200" vault auth enable kubernetes

  # Get the kubernetes secret associated with this account
  service_secret=`kubectl --kubeconfig /usr/local/bootstrap/kubeconfig get serviceaccount vault-auth -o json | jq -Mr '.secrets[].name'`

  # Then use the secret to get the Service JWT
  service_jwt=`kubectl --kubeconfig /usr/local/bootstrap/kubeconfig get secrets ${service_secret} -o json | jq -Mr '.data.token' | base64 -d`

  # We also need to locate the CA certificate used by K8S
  kubectl --kubeconfig /usr/local/bootstrap/kubeconfig get secrets ${service_secret} -o json | jq -Mr '.data["ca.crt"]' | base64 -d > /usr/local/bootstrap/k8sca.crt

  # Enable kubernetes backend in vault
  sudo VAULT_TOKEN=${VAULT_TOKEN} VAULT_ADDR="http://${VAULT_IP}:8200"  vault write auth/kubernetes/config \
    token_reviewer_jwt="${service_jwt}" \
    kubernetes_host=https://192.168.2.9:6443 \
    kubernetes_ca_cert=@/usr/local/bootstrap/k8sca.crt
...
deploy_demo_application () {
    ...
    echo "curl -kL http://192.168.2.9/health"
    echo "curl -kL http://192.168.2.9/initialiseme"
    echo "curl -kL http://192.168.2.9/approlename"
    echo "=====================The End============================="
}
